...
function writableStreamMarkFirstWriteRequestInFlight(stream) {
  ...
  assert(!stream[kState].writeRequests.isEmpty());
...
function writableStreamFinishErroring(stream) {
  ...
  for (const writeRequest of stream[kState].writeRequests)
    writeRequest.reject?.(storedError);
  stream[kState].writeRequests = new FixedQueue();
