...
class ReadableStream {
  ...
  values(options = kEmptyObject) {
    ...
    async function returnSteps(value) {
      ...
      assert(reader[kState].readRequests.isEmpty());
      if (!preventCancel) {
...
class ReadableStreamDefaultReader {
  ...
  [kInspect](depth, options) {
    return customInspect(depth, options, this[kType], {
      ...
      readRequests: this[kState].readRequests.size(),
...
class ReadableStreamBYOBReader {
  ...
  constructor(stream) {
    ...
    this[kState] = {
      stream: undefined,
      readIntoRequests: new FixedQueue(),
      close: {
        promise: undefined,
        resolve: undefined,
        reject: undefined,
  ...
  [kInspect](depth, options) {
    return customInspect(depth, options, this[kType], {
      stream: this[kState].stream,
      readIntoRequests: this[kState].readIntoRequests.size(),
      close: this[kState].close.promise,
    });
  }
}
...
class ReadableByteStreamController {
  ...
  get byobRequest() {
    ...
    if (this[kState].byobRequest === null &&
        !this[kState].pendingPullIntos.isEmpty()) {
      const {
        buffer,
        byteOffset,
        bytesFilled,
  ...
  [kRelease]() {
    ...
    if (!pendingPullIntos.isEmpty()) {
      ...
      this[kState].pendingPullIntos = new FixedQueue();
      this[kState].pendingPullIntos.push(firstPendingPullInto);
    }
  }
  ...
  function pullWithDefaultReader() {
    ...
    const readRequest = {
      ...
      [kClose]() {
        ...
        if (!branch1[kState].controller[kState].pendingPullIntos.isEmpty()) {
          readableByteStreamControllerRespond(branch1[kState].controller, 0);
        }
        if (!branch2[kState].controller[kState].pendingPullIntos.isEmpty()) {
    ...
    const readIntoRequest = {
      ...
      [kClose](chunk) {
        ...
        if (chunk !== undefined) {
          ...
          if (
            !otherCanceled &&
            !otherBranch[kState].controller[kState].pendingPullIntos.isEmpty()
          ) {
...
function readableStreamCancel(stream, reason) {
  ...
  if (reader !== undefined && readableStreamHasBYOBReader(stream)) {
    for (const readIntoRequest of reader[kState].readIntoRequests)
      readIntoRequest[kClose]();
    reader[kState].readIntoRequests = new FixedQueue();
  }
...
function readableStreamClose(stream) {
  ...
  if (readableStreamHasDefaultReader(stream)) {
    for (const readRequest of reader[kState].readRequests)
      readRequest[kClose]();
    reader[kState].readRequests = new FixedQueue();
  }
}
...
function readableStreamGetNumReadRequests(stream) {
  ...
  return stream[kState].reader[kState].readRequests.size();
}
...
function readableStreamGetNumReadIntoRequests(stream) {
  assert(readableStreamHasBYOBReader(stream));
  return stream[kState].reader[kState].readIntoRequests.size();
}
...
function readableStreamFulfillReadIntoRequest(stream, chunk, done) {
  ...
  assert(!reader[kState].readIntoRequests.isEmpty());
...
function readableStreamDefaultControllerClose(controller) {
  ...
  if (controller[kState].queue.isEmpty()) {
...
function readableStreamDefaultControllerPullSteps(controller, readRequest) {
  ...
  if (!queue.isEmpty()) {
    ...
    if (controller[kState].closeRequested && queue.isEmpty()) {
      readableStreamDefaultControllerClearAlgorithms(controller);
      readableStreamClose(stream);
    } else {
      readableStreamDefaultControllerCallPullIfNeeded(controller);
...
function readableByteStreamControllerClearPendingPullIntos(controller) {
  ...
  controller[kState].pendingPullIntos = new FixedQueue();
}
...
function readableByteStreamControllerFillHeadPullIntoDescriptor(
  ...
  assert(pendingPullIntos.isEmpty() || pendingPullIntos.peek() === desc);
...
function readableByteStreamControllerProcessPullIntoDescriptorsUsingQueue(
  ...
  const filledPullIntos = [];
  while (!pendingPullIntos.isEmpty()) {
    if (!controller[kState].queueTotalSize)
      break;
    const desc = pendingPullIntos.peek();
    if (readableByteStreamControllerFillPullIntoDescriptorFromQueue(
...
function readableByteStreamControllerProcessReadRequestsUsingQueue(controller) {
  ...
  while (!reader[kState].readRequests.isEmpty()) {
    if (queueTotalSize === 0) {
      return;
    }
...
function setupReadableByteStreamController(
  ...
  controller[kState] = {
    ...
    cancelAlgorithm,
    autoAllocateChunkSize,
    pendingPullIntos: new FixedQueue(),
  };
  stream[kState].controller = controller;
